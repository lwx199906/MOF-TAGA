...
# 读取CSV数据
df = pd.read_csv(r"database\基因型.csv")
...
print(f"\n✅ 已保存模型到：{model_path}")
print(f"✅ 已保存训练集预测结果到：{os.path.join(output_dir, 'train_predictions.csv')}")
print(f"✅ 已保存测试集预测结果到：{os.path.join(output_dir, 'test_predictions.csv')}")
